...
(describe "pet-eglot--adjust-paths-advice"
  (it "should adjust paths to find Python LSP servers"
    (spy-on 'eglot-ensure :and-call-fake (lambda (&rest args) (string-join args " ")))
    (spy-on 'pet-adjust-paths-executable-find :and-call-fake 'identity)

    (expect (pet-eglot--adjust-paths-advice) :to-equal nil)
    (expect (spy-context-return-value (spy-calls-most-recent 'pet-adjust-paths-executable-find)) :to-equal "pylsp")
    (expect 'eglot-ensure :not :to-have-been-called)))
...
(describe "pet-eglot-setup"
  ...
  (it "should advice eglot functions"
    ...
    (expect (advice-member-p 'pet-eglot--adjust-paths-advice 'eglot-ensure) :to-be-truthy)
...
(describe "pet-eglot-teardown"
  (it "should remove `pet' advices from eglot functions"
    ...
    (expect (advice-member-p 'pet-eglot--adjust-paths-advice 'eglot-ensure) :to-be nil)
