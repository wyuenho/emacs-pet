...
(describe "pet-def-use-env-tool macro"

  (before-all
    (defun my-test-checker ()
      "A dummy checker that returns a fixed path."
      "/path/to/dummy-tool")

    (pet-def-use-env-tool dummy-tool
      "A test function generated by the macro."
      (my-test-checker)))

  (after-all
    (fmakunbound 'my-test-checker)
    (fmakunbound 'pet-use-dummy-tool-p))

  (before-each
    (spy-on 'my-test-checker :and-call-through)
    (spy-on 'pet-project-root :and-return-value "/fake/project")
    (clrhash pet--env-tool-path-cache)
    (setq-local pet-memoization-ttl 300))

  (after-each
    (kill-local-variable 'pet-memoization-ttl))

  (it "should create a function with the correct name"
    (expect (fboundp 'pet-use-dummy-tool-p) :to-be t))

  (it "should cache the result on the first call"
    (let ((result1 (pet-use-dummy-tool-p))
          (result2 (pet-use-dummy-tool-p)))

      (expect result1 :to-equal "/path/to/dummy-tool")
      (expect result2 :to-equal "/path/to/dummy-tool")

      (expect 'my-test-checker :to-have-been-called-times 1)))

  (it "should re-execute the body after TTL expires"
    (setq-local pet-memoization-ttl 5)

    (expect (pet-use-dummy-tool-p) :to-equal "/path/to/dummy-tool")
    (expect 'my-test-checker :to-have-been-called-times 1)

    (spy-on 'time-since :and-return-value (seconds-to-time 10))

    (expect (pet-use-dummy-tool-p) :to-equal "/path/to/dummy-tool")

    (expect 'my-test-checker :to-have-been-called-times 2))

  (it "should not share cache between different projects"
    (pet-use-dummy-tool-p)

    (expect 'my-test-checker :to-have-been-called-times 1)

    (spy-on 'pet-project-root :and-return-value "/project/two")
    (pet-use-dummy-tool-p)

    (expect 'my-test-checker :to-have-been-called-times 2))

  (it "should not cache when TTL is 0"
    (setq-local pet-memoization-ttl 0)
...
(describe "pet-use-pre-commit-p"
  ...
  (describe "when the project has a `.pre-commit-config.yaml' file"
    ...
    (it "should return `pre-commit' path if `pre-commit' is found globally"
      (spy-on 'executable-find :and-return-value "/usr/bin/pre-commit")
      (expect (pet-use-pre-commit-p) :to-equal "/usr/bin/pre-commit"))

    (it "should return virtualenv `pre-commit' path when global search fails"
